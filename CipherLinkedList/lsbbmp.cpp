...
//	printf("%d", var->isipesan);
    ...
    //int messageLength = sizeof(En->pesanEncrypt) / sizeof(En->pesanEncrypt[0]);
	 // Jumlah byte termasuk huruf null terminator
    ...
    sprintf(str, "%d", messageLength); // Menyimpan messageLength sebagai huruf ke dalam str
//    printf("\nprint str %s\n", str); // Cetak str sebagai string, bukan sebagai satu huruf
    ...
    for (int i = 0; i < img->width * img->height; i++) {
 	for (int bit = 0; bit < 2; bit++) {
            if (bitIndex < len) {
                ...
                *color = binaryMessage[bitIndex++] - '0';
	    	}
		}
	}
}
...
void extractlsb(const BMPImage *img, int len, int panjang, char hasilLSB[]) {
    ...
    for (int i = 12; i < len; i+=4) {
   	...
   	printf("Hasil setelah hasil bmp : ");
